...
Prepares the recursive calls to `isf_allsv_nH_Wechelon_rec!`
Modifies info through a recursion on the size of the current subset.

More precisions are found in `isf_allsv_nH_Wechelon_rec.jl`
The 'symmetric' (H) and 'compact' (HnH) versions are in their respective 
functions (files) `isf_allsv_H_Wechelon!(.jl)`, `isf_allsv_HnH_Wechelon!(.jl)`.

For additional details, see `isf_allsv_nH(.jl)`, which is a more natural 
introduction, but often slightly slower.
"""
function isf_allsv_nH_Wechelon!(Vt::Matrix, perm::Vector{Int64}, info::Info, options::Options, values::Values)

    p = size(Vt, 2)
    n = size(Vt, 1) - 1
    if p < 3
        println("isf_allsv_nH_Wechelon!: function is not made for small values of p = $(p)")
...
# ----------------------------------------------------------------------------------------------------
    ...
    if options.rational                                                 # test before to avoid doing it every time
        for i in 1:p
            isf_allsv_nH_Wechelon_rec_r!(Vt, [i], perm, echelonned_form(Vt[1:n,i]'), info, options)
        end
    else
        for i in 1:p
            isf_allsv_nH_Wechelon_rec!(Vt, [i], perm, echelonned_form(Vt[1:n,i]'), info, options)
        end
    end
